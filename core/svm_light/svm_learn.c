...
void svm_learn_classification(DOC **docs, double *aclass, long int
		     ...
		     optimization if not NULL. Array must be of size totdoc. */
{
  ...
  if(kernel_parm->kernel_type == SVM_LINEAR) {
    /* kernel_cache = NULL; */
  } 
  ...
  if(alpha) {
    ...
    if(kernel_cache && (kernel_parm->kernel_type != SVM_LINEAR)) {
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  /* caching makes no sense for linear kernel */
  if((kernel_parm->kernel_type == SVM_LINEAR) && (*kernel_cache)) {
...
/* The following solves a freely defined and given set of
   ...
   This corresponds to the -z o option. */
     ...
     /* alpha:       Start values for the alpha variables or NULL
	             pointer. The new alpha values are returned after 
		     optimization if not NULL. Array must be of size totdoc. */
void svm_learn_optimization(DOC **docs, double *rhs, long int totdoc, long int totwords, LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm, KERNEL_CACHE *kernel_cache, MODEL *model, double *alpha) {
     ...
     /* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  for(;retrain && (!terminate);iteration++) {
    ...
    if((!retrain) && (inactivenum>0) 
	   ...
	   || (kernel_parm->kernel_type == SVM_LINEAR))) { 
      if(((verbosity>=1) && (kernel_parm->kernel_type != SVM_LINEAR)) 
	 || (verbosity>=2)) {
	if(verbosity==1) {
	  printf("\n");
	}
	...
	retrain=identify_one_misclassified(lin,label,unlabeled,totdoc,
      ...
      if(retrain) {
	if(kernel_parm->kernel_type == SVM_LINEAR) { /* reinit shrinking */
	  learn_parm->epsilon_crit=2.0;
	} 
      }
      if(verbosity>=1) {
     ...
     /* Check KT-conditions */
{
  ...
  if(kernel_parm->kernel_type == SVM_LINEAR) {  /* be optimistic */
        ...
        minshrink variables can be removed. */
{
  ...
  if((change>=minshrink) /* shrink only if sufficiently many candidates */
    ...
    if(kernel_parm->kernel_type != SVM_LINEAR) { /*  non-linear case save alphas */
      a_old=(double *)my_malloc(sizeof(double)*totdoc);
 	         ...
 	         values for linear kernel! */
{
  ...
  if(kernel_parm->kernel_type == SVM_LINEAR) { /* special linear case */
  ...
  if(kernel_parm->kernel_type != SVM_LINEAR) { /* update history for non-linear */
    for(i=0;i<totdoc;i++) {
