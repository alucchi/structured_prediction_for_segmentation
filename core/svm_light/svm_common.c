     ...
     /* classifies one example */
{
  ...
  if((model->kernel_parm.kernel_type == SVM_LINEAR) && (model->lin_weights))
     ...
     /* calculate the kernel function */
{
  ...
  if(kernel_parm->kernel_type == SVM_GRAM) {  /* use value from explicitly */
     ...
     /* calculate the kernel function between two vectors */
{
  ...
  switch(kernel_parm->kernel_type) {
    ...
    case SVM_SIGMOID:/* sigmoid neural net */
            return(tanh(kernel_parm->coef_lin*sprod_ss(a,b)+kernel_parm->coef_const)); 
    case SVM_CUSTOM: /* custom-kernel supplied in file kernel.h*/
            return(custom_kernel(kernel_parm,a,b)); 
    default: printf("Error: Unknown kernel function\n"); exit(1);
  }
}

SVECTOR *create_svector(SWORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
     ...
     /* compute length of weight vector */
{
  ...
  if(model->kernel_parm.kernel_type != SVM_LINEAR) {
     ...
     /* compute weight vector in linear case and add to model */
{
...
#ifdef DEBUG_AL
  char buf[100];
  sprintf(buf,"SVs_%04d.txt", iter);
  FILE *fp = fopen(buf, "w");
  for(i=1;i<model->sv_num;i++) {
    fprintf(fp, "%g\n", model->alpha[i]);
    for(f=(model->supvec[i])->fvec;f;f=f->next) {
      //fprintf(fp, "%g ", f->factor);
      SWORD *ai;
      ai = f->words;
      while (ai->wnum) {
  ...
  if(model->sv_num > 1) {
    SWORD *ai;
    ai = model->supvec[1]->fvec->words;
    fprintf(fp, "w:\n");
    while (ai->wnum) {
      fprintf(fp, "%d:%g\n", ai->wnum, model->lin_weights[ai->wnum]);
      ai++;
    }
  }
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  if(0 && model->kernel_parm.kernel_type == SVM_LINEAR) {
    if(verbosity>=1) {
