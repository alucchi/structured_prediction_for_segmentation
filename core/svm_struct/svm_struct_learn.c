...
// If UPDATE_C is defined, then C will be updated until it reaches a maximum value
//#define UPDATE_C
              ...
              STRUCTMODEL *sm, int alg_type)
{
  ...
  double      rt1,rt2;
  LABEL       *ybar = (LABEL*)my_malloc(n*sizeof(LABEL));
  ...
  if(sparm->slack_norm == 1) {
    ...
    lparm->sharedslack=0;
    if(kparm->kernel_type != SVM_LINEAR) {
  ...
  // allocate memory for find_most_violated_constraint
  /*
  uint maxNbNodes = 0;
  for(int il=0; il<n; il++)
    {
      if(maxNbNodes < ex[il].y.nNodes)
        maxNbNodes = ex[il].y.nNodes;
    }

  const int maxThreads = 2;
  int **tempNodeLabels = new int*[maxThreads];
  for(int il=0; il<maxThreads; il++)
  {
    tempNodeLabels[il] = new int[maxNbNodes];
  }
  */
  ...
  do { /* iteratively increase precision */
    ...
    do { /* iteration until (approx) all SV are found for current
...
#endif
         for(int il=0; il<n; il++) { /*** precomputation step to generate ybars ***/

#ifdef USE_OPENMP
          int threadId = omp_get_thread_num();
          printf("[svm_struct_learn] Thread %d/%d\n", threadId,omp_get_num_threads());
#endif

          if(sparm->loss_type == SLACK_RESCALING) 
            ybar[il]=find_most_violated_constraint_slackrescaling(ex[il].x,
                                                                  ex[il].y,
                                                                  sm,
                                                                  sparm);
          else
            ybar[il]=find_most_violated_constraint_marginrescaling(ex[il].x,
                                                                   ex[il].y,
                                                                   sm,
                                                                   sparm);
        }
         ...
         for(i=0; i<n; i++) { /*** example loop ***/
           ...
           if((!use_shrinking) || (opti[i] != opti_round)) {
            ...
            if(slack > (margin-dist+0.0001)) {
              ...
              // save constraints before updating w
              stringstream sout;
              sout << "constraints_" << numIt;
              save_constraints(cset, sm, sout.str().c_str());

              // Dirty hack (program will end anyway)
              cset.m--;
              totconstraints--;
#endif
	  ...
	  if((newconstraints >= sparm->newconstretrain) 
	    ...
	    if(struct_verbosity>=1) {
	      printf("[svm_struct_learn] Running optimization. Starting time = %ld clocks = %f seconds\n", time_0, time_0/(float)CLOCKS_PER_SEC);
              fflush(stdout);
	    }
	    svm_learn_optimization(cset.lhs,cset.rhs,cset.m,sizePsi+n,
				   lparm,kparm,kcache,svmModel,alpha);
	    if(struct_verbosity>=1) {
	      ...
	      printf("[svm_struct_learn] Running optimization : DONE. Time = %ld clocks = %f seconds\n", t, t/(float)CLOCKS_PER_SEC);
              fflush(stdout);
	    }
	    if(kcache)
	      kernel_cache_cleanup(kcache);
			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  double      rhs=0;
  double      slack,ceps;
  double      dualitygap,modellength,alphasum,alphasum2;
  long        sizePsi;
  double      *alpha=NULL;
  ...
  */
  ...
  for(i=0;i<n;i++) {
    if(USE_FYCACHE) {
      fy=psi(ex[i].x,ex[i].y,sm,sparm);
      if(kparm->kernel_type == SVM_LINEAR) { /* store difference vector directly */
...
#ifdef UPDATE_C
  uint iter = 1;
  double C_init = sparm->C;
#endif

  /*
#ifdef USE_OPENMP
  int nThreads = NTHREADS;
  printf("[svm_struct_learn] Setting number of threads to %d\n", nThreads);
...
*/
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
      ...
      //printf("Computing slack cset.m=%d\n", cset.m);
      alphasum=0;
      for(j=0;(j<cset.m);j++) 
	  alphasum+=alpha[j];
      ...
      slack=MAX(0,slack);
      //printf("slack %g", slack);

      /*
      for(j=0;j<cset.m;j++)
        {
          printf("alpha[j]=%g alphasum/cset.m=%g\n", alpha[j], alphasum/cset.m);
          printf("slack=%g\n", cset.rhs[j]-classify_example(svmModel,cset.lhs[j]));
        }
      */
      ...
      rhs=0;
      if(alg_type == ONESLACK_DUAL_CACHE_ALG) {
        printf("ONESLACK_DUAL_CACHE_ALG\n");
        rt1=get_runtime();
        ...
        if(viol-slack > MAX(epsilon_est/10,sparm->epsilon)) { 
          ...
          for(j=0;(j<batch_size) || ((j<n)&&(viol-slack<sparm->epsilon));j++) {
            if(struct_verbosity>=1) 
              print_percent_progress(&progress,n,10,(char*)".");
            uptr=uptr % n;
            if(randmapping) 
              i=randmapping[uptr];
            else
	...
	rt_total+=MAX(get_runtime()-rt1,0);
        ...
        double *local_rhs = (double *)my_malloc(sizeof(double)*n);
        SVECTOR** local_fydelta = (SVECTOR **) my_malloc(sizeof(SVECTOR *)*n);
        //SVECTOR* local_fydelta[n];
        double *local_rt_viol = (double *)my_malloc(sizeof(double)*n);
        double *local_rt_psi = (double *)my_malloc(sizeof(double)*n);
        long int *local_argmax_count = (long int*)my_malloc(sizeof(long int)*n);
        double *local_rt1 = (double *)my_malloc(sizeof(double)*n);

        // iterates over all the samples.
	...
	for(int i=0; i<n; i++) {
          ...
          local_rt_viol[i] = 0;
          local_rt_psi[i] = 0;
          local_argmax_count[i] = 0;

	  local_rt1[i] = get_runtime();
...
#pragma omp critical
	  ...
	  find_most_violated_constraint(&local_fydelta[i],&local_rhs[i],&ex[i],fycache[i],
                                        n,sm,sparm,&local_rt_viol[i],&local_rt_psi[i],
                                        &local_argmax_count[i]);
        }

	for(int i=0; i<n; i++) {

	  rt_viol += local_rt_viol[i];
	  rt_psi +=  local_rt_psi[i];
	  argmax_count += local_argmax_count[i];

	  /* add current fy-fybar to lhs of constraint */
	  if(kparm->kernel_type == SVM_LINEAR) {
            //printf("kparm->kernel_type == SVM_LINEAR\n");
	    add_list_n_ns(lhs_n,local_fydelta[i],1.0); /* add fy-fybar to sum */
	    free_svector(local_fydelta[i]);
	  }
	  else {
	    append_svector_list(local_fydelta[i],lhs); /* add fy-fybar to vector list */
	    lhs=local_fydelta[i];
	  }

	  rhs+=local_rhs[i];                         /* add loss to rhs */
	   
          double r=MAX(get_runtime()-local_rt1[i],0);
	  rt_total+=r;
	...
	rt_total+=MAX(get_runtime()-rt1,0);
        ...
        free(local_rt_viol);
        free(local_rt_psi);
        free(local_argmax_count);
        free(local_rt1);
      } /* end of finding most violated joint constraint */

      rt1=get_runtime();

      printf("[SVM_struct_learn] old_slack=%g, rhs=%g, lhsXw=%g, old_slack > new_slack=(rhs-lhsXw+0.000001)=%d\n",
             slack, rhs, lhsXw, slack > (rhs-lhsXw+0.000001));
	...
	printf("slack=%f, newslack=%f\n",slack,rhs-lhsXw);
...
#ifdef UPDATE_C
      // check end of loop condition and update C if necessary
      if(ceps <= sparm->epsilon)
        {
          // update C
          sparm->C = min(sparm->C*10, sparm->maxC);
          lparm->svm_c = sparm->C;
          printf("[svm_struct] Updating C to %g, ceps=%g,sparm->epsilon=%g\n",
                 sparm->C,ceps,sparm->epsilon);
          
          ofstream ofs("C_update.txt", ios::app);
          ofs << iter << endl;
          ofs.close();
      ...
      if(sparm->C <= sparm->maxC)
        ...
        { 
          ...
          if((alg_type == ONESLACK_DUAL_ALG) 
             || (alg_type == ONESLACK_DUAL_CACHE_ALG))
            kparm->kernel_type=SVM_GRAM; /* use kernel stored in kparm */
      ...
      if(struct_verbosity>=1)
        ...
        printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f)\n",cset.m,
               svmModel->sv_num-1,ceps,svmModel->maxdiff);
        */
        printf("(Iteration=%d, NumConst=%d, SV=%ld, CEps=%.4f, rhs=%g, lhsXw=%g, old_slack=%g, new_slack=%g, QPEps=%.4f)\n",
               numIt, cset.m, svmModel->sv_num-1, ceps, rhs, lhsXw, slack, rhs-lhsXw, svmModel->maxdiff);
      ...
      for(int i=0; i<cset.m; i++)  
        alphasum2+=alpha[i]*cset.rhs[i];

      ofstream ofs("min.txt", ios::app);
      ofs << norm_w << " " << sparm->C << " " << slack << " " << viol << " ";
      ofs << 0.5*modellength*modellength+sparm->C*viol << " "; //primal
      ofs << alphasum2-0.5*modellength*modellength << "\n"; //dual
      ofs.close();

#if 0
      if(ceps > sparm->epsilon) {
        stringstream sout;
        sout << "constraints2_" << numIt;
        save_constraints(cset, sm, sout.str().c_str());
      }
#endif
          ...
          cached_constraint || (ceps > sparm->epsilon) || (sparm->C < sparm->maxC)) &&
          (sparm->nMaxIterations == -1 || numIt < sparm->nMaxIterations)
         );
#else
  ...
  printf("SSVM finished after %d/%d iterations. ceps=%g, sparm->epsilon=%g\n",
         numIt,sparm->nMaxIterations, ceps, sparm->epsilon);

  if(numIt >= sparm->nMaxIterations) {
    printf("Max number of iterations reached\n");
  }
  if(ceps <= sparm->epsilon) {
    printf("eps value reached\n");
  }

  if(struct_verbosity>=1) {
    printf("Final epsilon on KKT-Conditions: MAX(%.5f,%.5f)=%.5f\n",
           svmModel->maxdiff,ceps,MAX(svmModel->maxdiff,ceps));
    ...
    printf("Total number of constraints in final working set: %i (of %i)\n",(int)cset.m,(int)totconstraints);
    printf("Number of iterations: %d/%d\n",numIt,sparm->nMaxIterations);
    printf("Number of calls to 'find_most_violated_constraint': %ld\n",argmax_count);
    printf("Number of SV: %ld \n",svmModel->sv_num-1);
    printf("Norm of weight vector: |w|=%.5f\n",modellength);
  ...
  save_constraints(cset, sm, "constraints");
...
/**
 * cset is a set of linear inequality constrains: lhs[i]*w >= rhs[i]
 */
void save_constraints(CONSTSET& cset, STRUCTMODEL *sm, const char* output_filename)
{
  int j;
  SVECTOR *f;
  DOC* d;
  ...
  double max_viol = 0;
  double sum;
  double total_lhsXw = 0;
  double total_rhs = 0;

  ofstream ofs(output_filename);
  // c.set m is the total number of constrains
  ofs << cset.m << endl;
  /*
  for(j=0;j<cset.m;j++) {
    ofs << cset.rhs[j] << endl;
  ...
  */
  ...
  for(j=0;j<cset.m;j++) {
    ...
    ofs << d->docnum << endl;
    ofs << d->queryid << endl;
    ofs << d->costfactor << endl;
    ofs << d->slackid << endl;
    ofs << d->kernelid << endl;
    lhsXw = 0; // score
    sum = 0;
    for(f=d->fvec;f;f=f->next) {
      ofs << f->factor;
      ofs << " " << sm->w[0];
      fnum = 0;
      while(f->words[fnum].wnum) {
        ofs << " " << f->words[fnum].wnum << ":" << f->words[fnum].weight
            << ":" << sm->w[f->words[fnum].wnum];
        sum += sm->w[f->words[fnum].wnum]*f->words[fnum].weight;
        fnum++;
      }
      sum *= f->factor;
      lhsXw += sum;
    ...
    ofs << endl;
    ofs << "lhsXw = " << lhsXw << endl;
    ofs << "rhs = " << cset.rhs[j] << endl;
    viol = cset.rhs[j]-lhsXw;
    ofs << "viol = " << viol << endl;
    total_lhsXw += lhsXw;
    total_rhs += cset.rhs[j];
  ...
  ofs << "---------------\n";
  ofs << total_lhsXw << endl;
  ofs << total_rhs << endl;
  ofs << total_lhsXw/(double)cset.m << endl;
  ofs << total_rhs/(double)cset.m << endl;
  ofs << max_viol << endl;
...
void find_most_violated_constraint(SVECTOR **fydelta, double *rhs, 
                                   ...
                                   const STRUCTMODEL *sm, const STRUCT_LEARN_PARM *sparm,
        ...
        to the most violated constraint for example ex */
{
  ...
  static int iter = 0;
  char buf[100];
  sprintf(buf,"diff_%04d.txt", iter);
  FILE *fp = fopen(buf, "w");
  SWORD *ai;
  ai = fy->words;
  while (ai->wnum) {
    fprintf(fp, "%d:%g ", ai->wnum, ai->weight);
    ai++;
  }
  fprintf(fp, "\n");
  ai = fybar->words;
  ...
  SWORD *ai2 = fybar->words;
  ai = fy->words;
  while (ai->wnum) {
    fprintf(fp, "%d:%g ", ai->wnum, ai->weight-ai2->weight);
    ai++;
    ai2++;
  }
  fprintf(fp, "\n");
        ...
        run. */
{
  ...
  if(viol_gain > gainthresh) {
    fydelta_new=fydelta;
    if(struct_verbosity>=2) rt2=get_runtime();
    if(svmModel->kernel_parm.kernel_type == SVM_LINEAR) {
...
{ 
  ...
  for(i=0; i<ccache->n; i++) { /*** example loop ***/
    ...
    if(struct_verbosity>=3) 
      print_percent_progress(&progress,ccache->n,10,(char*)"+");
